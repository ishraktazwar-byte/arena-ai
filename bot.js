...
try {
  ...
  runtime = attachRuntime(bot, emit, { provider, identity, aiIntervalMs: config.aiIntervalMs, memory, miningPolicy: config.miningPolicy, workspacePolicy: config.workspacePolicy, collectionPolicy: config.collectionPolicy });
  ...
  emit({ type: 'START', version: '0.2.6', message: 'Commands: status, step, stop, quit. Persistent local memory enabled; cloud planning opt-in. Live validation pending.' });
} catch (error) {
