...
export function chooseFarmAction(bot, intent, policies, available = () => true) {
  const p = bot.entity?.position;
  if (!validFarm(intent) || !intent) return { status: 'inactive' };
  if (!p || bot.version !== '1.21.1' || bot._client?.state !== 'play' || !bot.entity.onGround || bot.health < 12 || bot.food < 12 || assessRisk(survivalSnapshot(bot)).mode !== 'NORMAL' || !farmFootprint(worldReader(bot), p)) return { status: 'unsafe' };
  ...
  const spec = FARM_CROPS[intent.crop], seeds = inv.counts[spec.seed] || 0, stock = inv.counts[spec.produce] || 0;
  ...
  if (empty.length) {
    if (!seeds) return { status: 'seed_shortage' };
  ...
  return { status: cells.some(cell => cell.ripe) ? 'harvest_blocked' : 'waiting_for_growth' };
}
export class FarmManager {
  constructor({ bot, memory, policies, execute, emit = () => {}, now = Date.now }) {
    ...
    attachSeedReserve(bot, () => {
      ...
      return farm && this.active && permitsBlock(this.policies.farming, this.bot.game?.dimension, farm) && [this.policies.farming, this.policies.collection, this.policies.navigation].every(p => p.enabled) ? { ...farm, seed: FARM_CROPS[farm.crop].seed, count: farm.reserve } : null;
    });
  }
  intent() { return this.memory?.retrieveFarm({ dimension: this.bot.game?.dimension }) || null; }
  status() { return { intent: this.intent(), state: this.lastStatus || 'inactive', recovery: 'fresh_world_reconciliation' }; }
  ...
  async setGoal(farm, session) {
    ...
    const guard = () => session.guard(() => {
      ...
      if (farm && (!this.policies.farming.enabled || !this.policies.collection.enabled || !this.policies.navigation.enabled || !permitsBlock(this.policies.farming, dimension, farm) || !entity?.position || distance(entity.position, farm) > 6 || Math.round(entity.position.y) !== farm.y || readBlock(this.bot, { x: farm.x, y: farm.y - 1, z: farm.z })?.name !== 'farmland')) throw new FarmingError('farm_site_unavailable');
    });
    guard();
    ...
    guard(); this.failures.clear(); this.nextAt = 0;
  ...
  async runTick() {
    ...
    try {
      ...
      const decision = chooseFarmAction(this.bot, intent, this.policies, g => (this.failures.get(scope + key(g))?.until || 0) <= this.now());
      ...
      this.emit({ type: 'FARM-WORK', tool: decision.goal.tool, state: result.state });
