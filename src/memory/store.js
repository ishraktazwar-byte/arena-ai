...
const TOOLS = new Set(['scan', 'wait', 'move_step']);
...
async function load(path, agent) {
  ...
  if (Number.isInteger(envelope?.schemaVersion) && envelope.schemaVersion !== SCHEMA) throw new MemoryError('memory_schema_unsupported');
  if (!keysAre(envelope, ['schemaVersion', 'agent', 'records']) || envelope.schemaVersion !== SCHEMA || envelope.agent !== agent || !Array.isArray(envelope.records) || envelope.records.length > LIMIT || !envelope.records.every(validRecord)) throw new MemoryError('memory_corrupt');
