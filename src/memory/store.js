...
const SCHEMA = 4; // v1-v3 remain readable; workspace outcomes require v4 on write.
const LIMIT = 500;
const MAX_BYTES = 2 * 1024 * 1024;
const KINDS = new Set(['spawn', 'death', 'observation', 'goal_result']);
const STATES = new Set(['COMPLETED', 'BLOCKED', 'CANCELLED', 'FAILED']);
const TOOLS = new Set(['scan', 'wait', 'move_step', 'scan_resources', 'mine', 'craft_options', 'craft', 'workspace_options', 'place_crafting_table', 'craft_at_table']); // Historical names remain readable even if a tool is disabled.
...
async function load(path, agent) {
  ...
  if (Number.isInteger(envelope?.schemaVersion) && ![1, 2, 3, SCHEMA].includes(envelope.schemaVersion)) throw new MemoryError('memory_schema_unsupported');
  if (!keysAre(envelope, ['schemaVersion', 'agent', 'records']) || ![1, 2, 3, SCHEMA].includes(envelope.schemaVersion) || envelope.agent !== agent || !Array.isArray(envelope.records) || envelope.records.length > LIMIT || !envelope.records.every(validRecord)) throw new MemoryError('memory_corrupt');
