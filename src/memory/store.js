...
const SCHEMA = 7; // v1-v6 remain readable; navigation outcomes require v7 on write.
const LIMIT = 500;
...
async function load(path, agent) {
  ...
  if (Number.isInteger(envelope?.schemaVersion) && ![1, 2, 3, 4, 5, 6, SCHEMA].includes(envelope.schemaVersion)) throw new MemoryError('memory_schema_unsupported');
  if (!keysAre(envelope, ['schemaVersion', 'agent', 'records']) || ![1, 2, 3, 4, 5, 6, SCHEMA].includes(envelope.schemaVersion) || envelope.agent !== agent || !Array.isArray(envelope.records) || envelope.records.length > LIMIT || !envelope.records.every(validRecord)) throw new MemoryError('memory_corrupt');
