import { validFarm } from '../farming/intent.js';
...
export class MemoryStore {
  ...
  retrieveFarm({ dimension } = {}) {
    ...
    const record = this.records.find(r => r.kind === 'farm_intent' && r.worldId === this.worldId && r.dimension === dimension);
    return record && record.at <= this.now() ? structuredClone(record.data.farm) : null;
  }
