...
export const catalog = Object.freeze(Object.entries(definitions).filter(([name]) => !['mine', 'place_crafting_table', 'craft_at_table', 'collect_items', 'navigate_local', 'harvest_crop'].includes(name)).map(([name, definition]) => ({ name, ...definition })));
