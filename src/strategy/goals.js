...
export const catalog = Object.freeze(Object.entries(definitions).filter(([name]) => !['mine', 'place_crafting_table', 'craft_at_table', 'collect_items', 'collect_nearby', 'navigate_local', 'navigate_farm', 'manage_farm', 'stop_farm', 'harvest_crop', 'plant_crop'].includes(name)).map(([name, definition]) => ({ name, ...definition })));
