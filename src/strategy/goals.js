...
export const catalog = Object.freeze(Object.entries(definitions).filter(([name]) => !['mine', 'place_crafting_table', 'craft_at_table', 'collect_items', 'collect_nearby', 'navigate_local', 'navigate_farm', 'establish_farm', 'till_soil', 'dig_irrigation', 'fill_water_bucket', 'irrigate_basin', 'place_farm_block', 'fertilize_crop', 'cook_food', 'smelt_iron', 'manage_farm', 'stop_farm', 'harvest_crop', 'plant_crop'].includes(name)).map(([name, definition]) => ({ name, ...definition })));
