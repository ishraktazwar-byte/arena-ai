...
test('schema-v6 resource and event history migrates before storing navigation outcomes', async t => {
  const f = await fixture(t);
  await f.store.remember('spawn', { ...observation, nearbyResources: [{ name: 'oak_log', position: { x: 11, y: 64, z: 20 }, visibility: 'line_of_sight_sampled' }] });
  await f.store.close();
  const path = join(f.directory, 'memory.json');
  const prior = JSON.parse(await readFile(path, 'utf8')); prior.schemaVersion = 6;
  await writeFile(path, JSON.stringify(prior));
  const reopened = await MemoryStore.open(f.settings);
  try {
    await reopened.remember('goal_result', observation, { tool: 'navigate_local', state: 'CANCELLED' });
    assert.equal(JSON.parse(await readFile(path, 'utf8')).schemaVersion, 12);
  } finally { await reopened.close(); }
  const again = await MemoryStore.open(f.settings);
  try {
    assert.ok(again.retrieve(observation).some(record => record.data.tool === 'navigate_local' && record.data.state === 'CANCELLED'));
    assert.equal(again.retrieveResources(observation)[0].block, 'oak_log');
  }
  finally { await again.close(); }
});
test('schema-v8 objectives migrate before storing farming outcomes and crop-supply targets', async t => {
  const f = await fixture(t); await f.store.remember('objective', observation, { objective: { item: 'carrot', count: 4 } }); await f.store.close();
  const path = join(f.directory, 'memory.json');
  const prior = JSON.parse(await readFile(path, 'utf8')); prior.schemaVersion = 8;
  await writeFile(path, JSON.stringify(prior));
  const reopened = await MemoryStore.open(f.settings);
  try {
    assert.equal(reopened.retrieveObjective(observation).item, 'carrot');
    await reopened.remember('goal_result', observation, { tool: 'scan_crops', state: 'COMPLETED' });
    await reopened.remember('goal_result', observation, { tool: 'harvest_crop', state: 'FAILED' });
    await reopened.remember('objective', observation, { objective: { item: 'wheat_seeds', count: 8 } });
    assert.equal(JSON.parse(await readFile(path, 'utf8')).schemaVersion, 12);
  } finally { await reopened.close(); }
  const again = await MemoryStore.open(f.settings);
  try {
    assert.equal(again.retrieveObjective(observation).item, 'wheat_seeds');
    assert.ok(again.retrieve(observation).some(record => record.data.tool === 'harvest_crop'));
  } finally { await again.close(); }
});
test('schema-v9 farming history migrates before storing planting outcomes', async t => {
  const f = await fixture(t); await f.store.remember('goal_result', observation, { tool: 'harvest_crop', state: 'COMPLETED' }); await f.store.close();
  const path = join(f.directory, 'memory.json');
  const prior = JSON.parse(await readFile(path, 'utf8')); prior.schemaVersion = 9;
  await writeFile(path, JSON.stringify(prior));
  const reopened = await MemoryStore.open(f.settings);
  try {
    await reopened.remember('goal_result', observation, { tool: 'plant_crop', state: 'CANCELLED' });
    assert.equal(JSON.parse(await readFile(path, 'utf8')).schemaVersion, 12);
  } finally { await reopened.close(); }
  const again = await MemoryStore.open(f.settings);
  try {
    assert.ok(again.retrieve(observation).some(record => record.data.tool === 'plant_crop' && record.data.state === 'CANCELLED'));
    assert.ok(again.retrieve(observation).some(record => record.data.tool === 'harvest_crop' && record.data.state === 'COMPLETED'));
  }
  finally { await again.close(); }
});
test('schema-v10 planting history migrates while nearby collection results persist without executable targets', async t => {
  const f = await fixture(t); await f.store.remember('goal_result', observation, { tool: 'plant_crop', state: 'COMPLETED' }); await f.store.close();
  const path = join(f.directory, 'memory.json'), prior = JSON.parse(await readFile(path, 'utf8')); prior.schemaVersion = 10;
  await writeFile(path, JSON.stringify(prior));
  const reopened = await MemoryStore.open(f.settings);
  try {
    await reopened.remember('goal_result', observation, { tool: 'collect_nearby', state: 'COMPLETED', result: { entityId: 101, entityUuid: 'not-for-replay' } });
    assert.equal(JSON.parse(await readFile(path, 'utf8')).schemaVersion, 12);
  } finally { await reopened.close(); }
  const again = await MemoryStore.open(f.settings);
  try {
    assert.ok(again.retrieve(observation).some(r => r.data.tool === 'plant_crop'));
    const record = again.retrieve(observation).find(r => r.data.tool === 'collect_nearby');
    assert.deepEqual(record.data, { tool: 'collect_nearby', state: 'COMPLETED' });
  } finally { await again.close(); }
});
