...
test('legacy schema-v7 event history upgrades before adding objectives', async t => {
  ...
  const reopened = await MemoryStore.open(f.settings);
  try { await reopened.remember('objective', observation, { objective }); assert.equal((await disk(f)).schemaVersion, 9); assert.equal(reopened.retrieve(observation)[0].kind, 'spawn'); }
  finally { await reopened.close(); }
});
